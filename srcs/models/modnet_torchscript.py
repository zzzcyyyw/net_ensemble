...
class LRBranch(nn.Module):
    ...
    def forward(self, img, inference):
        ...
        pred_semantic = torch.tensor([1]) # None
        if not inference:
...
class HRBranch(nn.Module):
    ...
    def forward(self, img, enc2x, enc4x, lr8x, inference):
        #img2x = F.interpolate(img, scale_factor=1/2, mode='bilinear', align_corners=False, recompute_scale_factor=True)
        #img4x = F.interpolate(img, scale_factor=1/4, mode='bilinear', align_corners=False, recompute_scale_factor=True)
        ...
        pred_detail = torch.tensor([1]) # None
        if not inference:
