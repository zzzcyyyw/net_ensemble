...
class SalObjDataset(Dataset):
    def __init__(self, img_dir, mask_dir, transform=None):
        ...
        for img in os.listdir(img_dir):
            ...
            matte_name = img.split('--Fotor--')[0]
            matte_path = os.path.join(mask_dir, matte_name)

            self.imgs.append(file_path)
            self.masks.append(matte_path)
    ...
    def __getitem__(self, idx):
        ...
        #if imghdr.what(self.masks[idx]) == "png":
        #    Image.open(self.masks[idx]).convert("RGB").save(self.masks[idx])
